...
def getArgs():
    ...
    parser.add_argument('-pd',
                        '--pad',
                        help='''grow sampling location in km
    ...
    parser.add_argument('-sc',
                        ...
                        the data of interest otherwise mismatches in scale
                        will result in high pixel counts and a system error''',
                        default="30")
...
if __name__ == "__main__":
    ...
    if verbose:
        ...
        if location_file is not None:
            print("location file: {}".format(location_file))

        if location is not None:
            if location_file is not None:
                print("using location file and ignoring location")
            else:
                print("location: {}".format(location))
    ...
    if location_file is not None:
        if not os.path.isfile(location_file):
            sys.stderr.write("can't find location file\n")
            sys.exit(1)
        else:
            locations = pd.read_csv(location_file)
            print(locations)
    else:
        locations = pd.DataFrame(['site'] + location).transpose()
    ...
    for loc in locations.itertuples():
        ...
        # some feedback
        print("processing: " + loc[1] + " at " + "%s / %s" % (loc[3],
                                                              loc[2]))
            ...
            if args.verbose:
                ...
                if args.directory:
                    df.to_csv(args.directory + "/" +
                              loc[1] + "_" +
                              os.path.basename(args.product) +
                              "_gee_subset.csv", index=False)
                else:
                    print(df)
